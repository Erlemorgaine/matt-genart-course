...
const sketch = ({ context }) => {
  ...
  for (let i = 0; i < 40; i++) {
      // Setup a material. When you do BasicMaterial, it will work without lighting in the scene
    ...
    mesh.position.set(
      Math.random() * 2 - 1, Math.random() - 1, Math.random() - 1
    );
    mesh.scale.multiplyScalar(Math.random() * 0.15) // will multiply x, y, z by same number value
    scene.add(mesh);
  }
